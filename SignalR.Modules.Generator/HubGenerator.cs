...
");
        ...
        private string GetMethodParameters(IMethodSymbol methodSymbol)
        {
            return methodSymbol.Parameters.Length == 0
                ? "System.Array.Empty<object>()"
                : $"new[] {{ {string.Join(", ", methodSymbol.Parameters.Select(p => p.Name))} }}";
        }
        ...
        {{
            return SendAsync(""{methodSymbol.Name}"", {GetMethodParameters(methodSymbol)});
        }}";
        }
...
");
            ...
            {
                source.AppendLine($@"
        [SignalRSwaggerGen.Attributes.SignalRHidden]
